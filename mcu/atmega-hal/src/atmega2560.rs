...
impl_mod_port! {
    use crate::atmega2560 as hal;
    impl_port_peripheral_a8_b8_c8_d8_e8_f8_g6_h8_j8_k8_l8! {
    }

    #[macro_export]
    macro_rules! atmega2560_pins {
        ($p:expr) => {
            $crate::atmega2560::Pins::new($p.PORTA, $p.PORTB, $p.PORTC, $p.PORTD, $p.PORTE, $p.PORTF, $p.PORTG, $p.PORTH, $p.PORTJ, $p.PORTK, $p.PORTL)
        };
    }

    pub use atmega2560_pins as pins;
}
