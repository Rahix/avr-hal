...
pub mod port {
    ...
    /// ```
    #[macro_export]
    macro_rules! arduino_nano_v2_pins {
        ($p:expr) => {
            $crate::arduino::nano_v2::Pins::with_mcu_pins($crate::arduino::nano_v2::hal::pins!($p))
        };
    }

    pub use arduino_nano_v2_pins as pins;
}
...
pub mod usart {
    ...
    /// ```
    #[macro_export]
    macro_rules! arduino_nan_v3_default_serial {
        ($p:expr, $pins:expr, $baud:expr) => {
            $crate::arduino::nano_v2::Usart::new(
                $p.USART0,
                $pins.d0,
                $pins.d1.into_output(),
                $crate::arduino::nano_v2::hal::usart::BaudrateExt::into_baudrate($baud),
            )
        };
    }
    pub use arduino_nan_v3_default_serial as default_serial;
}
...
pub mod prelude {
    use crate::arduino::nano_v2 as board;
