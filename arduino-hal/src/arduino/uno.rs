...
pub mod port {
    ...
    /// ```
    #[macro_export]
    macro_rules! arduino_uno_pins {
        ($p:expr) => {
            $crate::arduino::uno::Pins::with_mcu_pins($crate::arduino::uno::hal::pins!($p))
        };
    }

    pub use arduino_uno_pins as pins;
}
...
pub mod usart {
    ...
    /// ```
    #[macro_export]
    macro_rules! arduino_uno_default_serial {
        ($p:expr, $pins:expr, $baud:expr) => {
            $crate::arduino::uno::Usart::new(
                $p.USART0,
                $pins.d0,
                $pins.d1.into_output(),
                // See comment in avr-hal-generic/src/usart.rs for why these boards use the
                // BaudrateArduinoExt trait instead of BaudrateExt
                $crate::arduino::uno::hal::usart::BaudrateArduinoExt::into_baudrate($baud),
            )
        };
    }

    pub use arduino_uno_default_serial as default_serial;
}
...
pub mod prelude {
    use crate::arduino::uno as board;
