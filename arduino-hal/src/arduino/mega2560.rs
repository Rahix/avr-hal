pub use atmega_hal::atmega2560 as hal;
...
pub mod port {
    ...
    /// ```
    #[macro_export]
    macro_rules! arduino_mega2560_pins {
        ($p:expr) => {
            $crate::arduino::mega2560::Pins::with_mcu_pins($crate::arduino::mega2560::hal::pins!($p))
        };
    }

    pub use arduino_mega2560_pins as pins;
}
...
pub mod usart {
    ...
    /// ```
    #[macro_export]
    macro_rules! arduino_mega2560_default_serial {
        ($p:expr, $pins:expr, $baud:expr) => {
            $crate::arduino::mega2560::Usart::new(
                $p.USART0,
                $pins.d0,
                $pins.d1.into_output(),
                // See comment in avr-hal-generic/src/usart.rs for why these boards use the
                // BaudrateArduinoExt trait instead of BaudrateExt
                $crate::arduino::mega2560::hal::usart::BaudrateArduinoExt::into_baudrate($baud),
            )
        };
    }
    pub use arduino_mega2560_default_serial as default_serial;
...
pub mod prelude {
    use crate::arduino::mega2560 as board;
